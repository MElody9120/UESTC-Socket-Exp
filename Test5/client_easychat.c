...
int	PrintTime(time_t time1,char *Time);
int send_msg(int fd);
...
int getUsername()
{
	...
	printf("		        By Melody	            \n");
	printf("------------------------------------\n");
	printf("Please Enter Your Username: (10 words)\n");
	
	fgets(chatID,sizeof(chatID),stdin);  //输入用户名
	
	if(strlen(chatID)<0){
		memset(chatID,0,sizeof(chatID));  
		return 1;
	}
	return 0;  
}
...
int PrintTime(time_t time,char *Time)
{
	struct tm tm1;
